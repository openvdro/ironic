...
Portgroups are supported since version 1.16 of ironic API, so it is added to
ironic CLI calls.
...
#. Create a neutron provisioning network and add it in ``/etc/ironic/ironic.conf``
   ...
   is used to swithing to cleaning/provisioning networks. For example set to neutron_plugin
...
#. A portgroup is a link aggregation, where multiple interfaces on the bare metal server
   ...
   Create a portgroup in ironic::
    ...
    ironic portgroup-create --ironic-api-version 1.16 -n $NODE_UUID \
    -a $NEW_MAC_ADDRESS --name $NAME

#. If using a portgroup, create a port as following::

    ironic --ironic-api-version 1.16 port-create -a $HW_MAC_ADDRESS \
...
#. Check the port configuration::
    ...
    ironic --ironic-api-version 1.16 port-show $PORT_UUID

#. Update portgroup membership::

    ironic --ironic-api-version 1.16 port-update $PORT_UUID replace \
