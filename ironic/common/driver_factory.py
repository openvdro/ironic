...
class BaseDriverFactory(object):
    ...
    def _init_extension_manager(cls):
        ...
        if cls._extension_manager:
            return
        conf_option = entrypoint_map.get(cls._entrypoint_name)
        enabled_drivers = (getattr(CONF, conf_option) if conf_option else [])
        ...
        cls._enabled_driver_list = list(item for (item, cnt) in counter)
