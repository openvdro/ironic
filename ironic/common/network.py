...
def add_ports_to_network(task, network_uuid, vif_field='vif_port_id'):
    """Create Neutron ports for each port on task.node to boot the ramdisk.

    :param task: a TaskManager instance.
    :param vif_field: name of the field where to put ID of a port created in
        Neutron. This is needed in case of cleaning, as Nova will remove
        vif_port_id from all the ironic ports some time after cleaning starts.
    :raises: NetworkError
    ...
    for ironic_port in task.ports:
        ...
        # Backup existing vif if vif_field has a default value.
        if vif_field == 'vif_port_id':
            vif = extra_dict.pop(vif_field, None)
            if vif:
        ...
        try:
            extra_dict[vif_field] = port['port']['id']
        except KeyError:
