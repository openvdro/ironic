...
class TestNeutron(db_base.DbTestCase):
    ...
    @mock.patch.object(network, 'add_ports_to_network')
    def test_create_cleaning_ports(self, add_ports_mock):
    ...
    @mock.patch.object(network, 'remove_ports_from_network')
    def test_delete_cleaning_ports(self, remove_ports_mock):
        api = dhcp_factory.DHCPFactory().provider
        with task_manager.acquire(self.context, self.node.uuid) as task:
