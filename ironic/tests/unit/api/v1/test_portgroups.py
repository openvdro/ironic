...
class TestListPortgroups(test_api_base.BaseApiTest):
    ...
    def test_ports_subresource_duplicated_portgroup_ident(self):
        pg = obj_utils.create_test_portgroup(self.context,
                                             uuid=uuidutils.generate_uuid(),
                                             node_id=self.node.id)

        response = self.get_json(
            '/portgroups/%s/ports?portgroup=abc' % pg.uuid,
            headers=self.headers, expect_errors=True)
        self.assertEqual(http_client.BAD_REQUEST, response.status_int)

        response = self.get_json(
            '/portgroups/%s/ports/detail?portgroup=abc' % pg.uuid,
            headers=self.headers, expect_errors=True)
        self.assertEqual(http_client.BAD_REQUEST, response.status_int)
    ...
    def test_ports_subresource_noid(self):
        ...
        obj_utils.create_test_port(self.context, node_id=self.node.id,
                                   portgroup_id=pg.id)
        # No portgroup id specified
        response = self.get_json('/portgroups/ports', expect_errors=True,
                                 headers=self.headers)
        self.assertEqual(http_client.BAD_REQUEST, response.status_int)
    ...
    def test_portgroup_by_address_invalid_api_version(self):
        address = "aa:bb:cc:dd:ee:f1"
        obj_utils.create_test_portgroup(
            self.context, node_id=self.node.id, uuid=uuidutils.generate_uuid(),
            name='portgroup_1', address=address)
        response = self.get_json('/portgroups?address=%s' % address,
...
class TestPost(test_api_base.BaseApiTest):
    ...
    @mock.patch.object(timeutils, 'utcnow')
    def test_create_portgroup_invalid_api_version(self, mock_utcnow):
        pdict = apiutils.post_get_test_portgroup()
        test_time = datetime.datetime(2000, 1, 1, 0, 0)
        mock_utcnow.return_value = test_time
    ...
    def test_create_portgroup_invalid_node_uuid_older_version(self):
        pdict = apiutils.post_get_test_portgroup(
